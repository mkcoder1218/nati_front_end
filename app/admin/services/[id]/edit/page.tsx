...
export default function AdminEditServiceGuidePage({ params }: EditServiceGuidePageProps) {
  ...
  useEffect(() => {
    if (guide) {
      setFormData({
        ...
        steps: guide.steps.length ? guide.steps : [{ title: "", description: "" }],
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await dispatch(updateServiceGuide({ 
        guideId, 
        data: cleanedData 
      })).unwrap();
      ...
      router.push(`/admin/services/${guideId}`);
    } catch (error) {
      toast({
        title: "Error",
    ...
    <div className="space-y-6">
      <div className="flex items-center gap-2">
        <Button variant="ghost" size="icon" asChild>
          <Link href={`/admin/services/${guideId}`}>
            <ArrowLeft className="h-4 w-4" />
          </Link>
        </Button>
        <h2 className="text-2xl font-bold tracking-tight">{t("edit_service")}</h2>
      </div>
          ...
          <Card>
            ...
            <CardContent className="space-y-6">
              <div className="space-y-2">
                ...
                <Select
                  ...
                  onValueChange={(value) => handleSelectChange("office_id", value)}
                >
                  <SelectTrigger>
              ...
              <div className="space-y-2">
                ...
                >
                  ...
                  <SelectContent>
                    ...
                    <SelectItem value="ID & Documentation">{t("id_documentation")}</SelectItem>
                    ...
                    <Card key={index}>
                      ...
                      <CardContent className="p-4 pt-0 space-y-2">
                        <div className="space-y-1">
                          <Label htmlFor={`step-title-${index}`}>{t("title")}</Label>
                          <Input
                            id={`step-title-${index}`}
                            value={step.title}
                        ...
                        <div className="space-y-1">
                          <Label htmlFor={`step-description-${index}`}>{t("description")}</Label>
                          <Textarea
            ...
            <CardFooter className="flex justify-between">
              <Button variant="outline" asChild>
                <Link href={`/admin/services/${guideId}`}>{t("cancel")}</Link>
              </Button>
