...
import { CreateServiceGuideData } from "@/services/serviceGuide.service";

export default function NewServiceGuidePage() {
  const { t } = useTranslation();
  const router = useRouter();
  const dispatch = useAppDispatch();
  const { toast } = useToast();

  const { loading } = useAppSelector((state) => state.serviceGuide);
  ...
  useEffect(() => {
    ...
    // If user has an office_id, pre-select it
    if (user?.office_id) {
      setFormData((prev) => ({
        ...prev,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast({
        title: "Success",
        description: "Service guide created successfully.",
      });
      router.push("/government/services");
    } catch (error) {
      toast({
        title: "Error",
        description: "Failed to create service guide. Please try again.",
        variant: "destructive",
      });
    }
  };
      ...
      <form onSubmit={handleSubmit}>
        <Card>
          <CardHeader>
            ...
            <CardDescription>{t("enter_service_details")}</CardDescription>
          </CardHeader>
          ...
          <CardFooter className="flex justify-between">
            <Button variant="outline" asChild>
              <Link href="/government/services">{t("cancel")}</Link>
            </Button>
            <Button type="submit" disabled={loading}>
              {loading ? t("creating") : t("create_service")}
            </Button>
          </CardFooter>
        </Card>
      </form>
