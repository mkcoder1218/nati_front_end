...
import { fetchUserVoteStats, fetchTopVotedOffices } from '@/store/slices/officeVoteSlice';
...
const OfficeVoteStats: React.FC<OfficeVoteStatsProps> = ({ className }) => {
  ...
  const { userVoteStats, topVotedOffices, statsLoading } = useAppSelector((state) => state.officeVote);
                ...
                {userVoteStats.voted_offices.length > 0 ? (
                  <ul className="space-y-2 text-sm">
                    {userVoteStats.voted_offices.slice(0, 5).map((vote) => (
                      <li key={vote.office_id} className="flex items-center justify-between">
                        <Link 
                  ...
                  <p className="text-sm text-muted-foreground">No recent votes.</p>
                )}
              </div>
            </div>
          )}
        ...
        <div className="space-y-4">
              ...
              <span className="font-medium">
                {topVotedOffices.length > 0 
                  ? Math.round(
                      ...
                      topVotedOffices.reduce((sum, office) => sum + office.total, 0)) * 100
                    ) 
                  : 0}%
              </span>
            </div>
          ...
          <div className="pt-2">
            ...
            {topVotedOffices.length > 0 ? (
                ...
                {topVotedOffices.slice(0, 5).map((office) => (
                  <li key={office.office_id} className="space-y-1">
                    <div className="flex justify-between items-center">
                      ...
                      <Badge 
                        variant={office.ratio >= 70 ? "success" : office.ratio >= 40 ? "warning" : "destructive"}
                        className="text-xs"
                      >
                        {office.ratio}%
                      </Badge>
              ...
              <p className="text-sm text-muted-foreground">No office vote data available.</p>
            )}
          </div>
        </div>
      </CardContent>
